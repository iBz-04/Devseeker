...
def cleanup():
    yield
    if Path(".devseeker_consent").exists():
        Path(".devseeker_consent").unlink()


"""
Test the following 4 scenarios for check_collection_consent():
    * The .devseeker_consent file exists and its content is "true".
    * The .devseeker_consent file exists but its content is not "true".
    * The .devseeker_consent file does not exist and the user gives consent when asked.
    * The .devseeker_consent file does not exist and the user does not give consent when asked.
"""


def test_check_consent_file_exists_and_true(cleanup):
    Path(".devseeker_consent").write_text("true")
    assert check_collection_consent() is True


def test_check_consent_file_exists_and_false(cleanup):
    Path(".devseeker_consent").write_text("false")
...
def test_check_consent_file_not_exists_and_user_says_yes(cleanup):
    with patch("builtins.input", side_effect=["y"]):
        assert check_collection_consent() is True
    assert Path(".devseeker_consent").exists()
    assert Path(".devseeker_consent").read_text() == "true"
...
Test the following 4 scenarios for ask_collection_consent():
    1. The user immediately gives consent with "y":
        * The .devseeker_consent file is created with content "true".
        * The function returns True.
    2. The user immediately denies consent with "n":
        * The .devseeker_consent file is not created.
